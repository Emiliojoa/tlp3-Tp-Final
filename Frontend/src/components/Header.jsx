...
export const Header = ({nombre, refs, onScrollTo}) => {
    const [newNombre, setNewNombre] = useState("Jugador")
     useEffect(() => {
    setNewNombre(nombre)
  }, [nombre])
  ...
  return (
    <div className='fixed w-full z-50 flex py-4 justify-between px-4 flex-1 bg-white'>
        <div className='flex flex-2 flex-row gap-3 items-center'>
            ...
            <button 
            onClick={() => onScrollTo(refs.inicioRef)}
            className="font-bold text-2xl text-black hover:scale-105 cursor-pointer hover:text-yellow-500">The goat</button>
        </div>
        <nav className="flex font-bold flex-10 justify-center gap-4 sm:gap-24 text-lg items-end">
            <article className="flex flex-col items-center">
                ...
                <button 
                onClick={() => onScrollTo(refs.precioRef)}
                className='hover:border-b-1 cursor-pointer'>Precio</button>
            </article>
            <article className="flex flex-col items-center">
                <Brain size={32} color="gray"/>
                <button 
                onClick={() => onScrollTo(refs.animoRef)}
                className='hover:border-b-1 cursor-pointer'>Mentality</button>
            </article>
        </nav>
